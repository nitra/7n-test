...
> [!IMPORTANT]
> **Worktree-only skill.** Запускай у окремому worktree `.worktrees/<current-branch>-taze/` і не паралель з іншими worktree-only скілами.

**Preflight:** у корені репо виконай `pwd`, `git rev-parse --show-toplevel`, `git branch --show-current`. Якщо `pwd` ≠ toplevel — спершу `cd` у корінь. Далі створи worktree через `npx @7n/mt worktree create "<current-branch>-taze" "n-taze: worktree-only skill"`, перейди в `.worktrees/<branch-with-dashes>-taze`, і після цього запусти `bun install`.
