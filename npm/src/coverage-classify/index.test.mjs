...
describe("classify", () => {
  ...
  const mockSurvived = [{
    file: "src/test.js",
    mutants: [{
      file: "src/test.js", line: 10, col: 1, replacement: "R1"
    }],
    exampleTest: null,
  }]
  ...
  it("should run Tier 1 classification if cache misses", async () => {
    const validVerdict = JSON.stringify({ verdict: 'worth-testing', confidence: 0.8, reason: 'This mutant changes core logic and needs a dedicated test to verify', suggestedTest: 'check it' })
  ...
  it("should run Tier 2 classification on Tier 1 failure", async () => {
    const validVerdict = JSON.stringify({ verdict: 'equivalent', confidence: 0.9, reason: 'This mutant is semantically equivalent to the original code' })
    const mockCallModel = vi.fn()
      .mockImplementationOnce(() => { throw new Error("Tier 1 Fail") })
