...
describe("coverage.mjs", () => {
  ...
  describe("renderMarkdown", () => {
    ...
    it("should render survived mutant without exampleTest", () => {
      const survived = [{ file: "a.js", mutants: [{ line: 1, col: 0, mutantType: "SR", original: "x", replacement: "y" }] }]
      const rows = [{ area: "r", coverage: { lines: { covered: 0, total: 1 }, functions: { covered: 0, total: 1 } }, mutation: { caught: 0, total: 1 }, survived }]
      const markdown = renderMarkdown(rows)
    ...
    it("should render allowed gaps section", () => {
      const rows = [{ area: "r", coverage: { lines: { covered: 5, total: 10 }, functions: { covered: 1, total: 2 } }, mutation: { caught: 5, total: 10 } }]
      const allowedGaps = [{
        file: "src/util.js",
    ...
    it("should sanitize pipe chars in verdict reason", () => {
      const rows = [{ area: "r", coverage: { lines: { covered: 1, total: 1 }, functions: { covered: 1, total: 1 } }, mutation: { caught: 1, total: 1 } }]
      const allowedGaps = [{
        file: "f.js",
